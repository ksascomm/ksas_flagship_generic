		...
		<section class="row" id="fields_container">
			...
			<?php foreach($roles as $role) {
				...
				if ($people_query->have_posts()) : ?>
					<li class="person sub-head <?php echo $role_classes; ?>"><h2 class="capitalize"><?php echo $role_name; ?></h2></li>
				<?php while ($people_query->have_posts()) : $people_query->the_post(); ?>
					<li class="person <?php echo get_the_roles($post); ?>">
						<div class="row">
							...
							<?php if ( get_post_meta($post->ID, 'ecpt_expertise', true) ) : ?><p><strong><?php echo $research_label; ?>:&nbsp;</strong><?php echo get_post_meta($post->ID, 'ecpt_expertise', true); ?></p><?php endif; ?>
						</div>
					</li>		
